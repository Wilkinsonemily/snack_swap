...
@php
    ...
    $cat = $p['category'] ?? null;
    if ($cat) {
        $cat = str_contains($cat, ':')
            ? ucwords(str_replace('-', ' ', explode(':', $cat, 2)[1]))
            : ucwords(str_replace('-', ' ', $cat));
    }
@endphp

<div class="col-12 col-sm-6 col-lg-4 col-xl-3">
    <div class="card shadow-sm border-0 search-card h-100">
        <img src="{{ $img }}" class="search-card-img" alt="{{ $p['name'] }}">

        <div class="card-body d-flex flex-column">
            <h6 class="fw-semibold mb-1 text-truncate" title="{{ $p['name'] }}">
                {{ $p['name'] }}
            </h6>
            ...
            <div class="mt-auto d-flex gap-2">
                <a href="{{ route('food.show', $p['id']) }}"
                   class="btn btn-outline-secondary btn-sm w-100">
                    Details
                </a>
                <a href="{{ route('swap.result', ['barcode' => $p['id']]) }}"
